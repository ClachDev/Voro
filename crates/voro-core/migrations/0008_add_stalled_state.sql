-- A real 'stalled' state (DESIGN.md §6): in-flight work whose dispatch died,
-- the third attention state beside 'needs-input' and 'review'. It replaces the
-- redispatch flag derived from session history, which had become behavioural
-- state living outside the state machine. SQLite cannot alter a CHECK
-- constraint, so the table is rebuilt, as in 0002; the runner disables
-- foreign_keys around migrations, and deps/sessions/events keep their task ids.
...
-- One-time backfill: a 'ready' task whose most recent session ended 'failed'
-- or 'capped' is exactly what the derived flag used to mark — it becomes
-- 'stalled'. 'state_since' is left alone so the age bonus keeps accruing from
-- when the task actually landed back in the queue.
...
WHERE state = 'ready'
  AND id IN (
    ...
    WHERE s.id = (SELECT MAX(id) FROM sessions WHERE task_id = s.task_id)
      AND s.outcome IN ('failed','capped')
  );
