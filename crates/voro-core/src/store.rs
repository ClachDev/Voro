...
const MIGRATIONS: &[&str] = &[
    ...
    include_str!("../migrations/0014_dep_kind_in_key.sql"),
];
...
mod tests {
    ...
    /// A database from before migration 0014 must open with every dependency
    /// edge intact, and accept a second edge of another kind between a pair the
    /// old primary key allowed only one edge for.
    #[test]
    fn migration_0014_widens_the_dep_key_without_losing_edges() {
        let conn = Connection::open_in_memory().unwrap();
