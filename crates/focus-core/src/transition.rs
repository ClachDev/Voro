...
pub enum Action {
    /// proposed → backlog | ready | rejected
    Triage(Triage),
    ...
    /// running → ready
    Abort,
    /// ready → backlog (deliberate parking)
    Park,
    /// backlog → ready (manual unpark)
    Unpark,
    /// backlog | ready | needs-input | review → rejected
    Abandon,
}
...
impl Store {
    ...
    pub fn apply(&mut self, task_id: i64, action: Action) -> Result<Task> {
        ...
        let to = match (task.state, &action) {
            (Proposed, Action::Triage(Triage::Backlog)) => Backlog,
            ...
            (Ready, Action::Park) => Backlog,
            (Backlog, Action::Unpark) => Ready,
            (Backlog | Ready | NeedsInput | Review, Action::Abandon) => Rejected,
            _ => {
...
/// - `backlog` with at least one blocker, all closed → promote to `ready`.
///   A backlog task with *no* blockers is deliberately parked and stays put.
/// - `ready` with an open blocker → demote to `backlog`.
pub(crate) fn reconcile_readiness(conn: &Connection, task_id: i64) -> Result<()> {
    ...
    let to = match task.state {
        TaskState::Backlog if total > 0 && open == 0 => TaskState::Ready,
        TaskState::Ready if open > 0 => TaskState::Backlog,
        _ => return Ok(()),
    };
    conn.execute(
...
mod tests {
    ...
    fn task_in_state(s: &mut Store, project_id: i64, state: TaskState) -> i64 {
        use TaskState::*;
        match state {
            Proposed | Backlog | Ready => create(s, project_id, state),
            Running => {
    ...
    fn expected(state: TaskState, action: &Action) -> Option<TaskState> {
        use TaskState::*;
        match (state, action) {
            (Proposed, Action::Triage(Triage::Backlog)) => Some(Backlog),
            ...
            (Ready, Action::Park) => Some(Backlog),
            (Backlog, Action::Unpark) => Some(Ready),
            ...
            (Backlog | Ready | NeedsInput | Review, Action::Abandon) => Some(Rejected),
            _ => None,
        }
    }
    ...
    fn accepting_last_blocker_promotes_dependant() {
        ...
        let dependant = create(&mut s, p, TaskState::Backlog);
    ...
    fn promotion_waits_for_the_last_blocker() {
        ...
        assert_eq!(s.task(dependant).unwrap().state, TaskState::Backlog);
    ...
    fn parked_task_without_blockers_is_never_auto_promoted() {
        let (mut s, p) = store_with_project();
        let parked = create(&mut s, p, TaskState::Backlog);
        let unrelated = task_in_state(&mut s, p, TaskState::Review);
        s.apply(unrelated, Action::Accept).unwrap();
        assert_eq!(s.task(parked).unwrap().state, TaskState::Backlog);
    }
    ...
    fn adding_open_blocker_demotes_ready_task() {
        ...
        assert_eq!(demoted.state, TaskState::Backlog);
