...
**Agent resolution**, in order: the task's `agent` override if set (the capability case — a task that needs image generation is inherently a task for the agent that has it, so the choice is worth persisting on the task); otherwise the global default from `agents.toml`. The TUI offers two dispatch actions: dispatch-with-resolved-agent on one key, dispatch-via-picker on another — the picker existing mostly for the cap case, where the default is temporarily unusable but nothing about the task has changed. Since `agents.toml` is not created for a fresh install, `voro agents init` scaffolds a starter file (refusing to clobber an existing one), `voro agents list` shows what is configured with the default flagged, and `voro agents path` prints where dispatch looks; the missing-config error points at `agents init` so the first failed dispatch is self-explaining.
...
**Continuation** is how an answer reaches the work. §6 says the answer to a `needs-input` question is appended to the body and logged, but stops short of saying how it gets back to the agent — and by the time a human answers, the asking session has typically already exited (outcome `asked`), so there is no live pipe to feed it to. `voro answer` (and the TUI's equivalent action) always performs the `needs-input → running` transition, and additionally dispatches a fresh session — the task body, now carrying the `## Answers` section, as its prompt — whenever the task has *any* prior session history. A task that was only ever started by hand has nothing to continue, and the answer stands alone as a plain transition; this is what keeps `answer` usable on tasks nothing has ever dispatched. Continuation reuses dispatch's own mechanics (agent resolution, the dirty-tree guard, prompt/log files) but not its precondition: it asserts the task is already `running` — `answer` just put it there — rather than performing the `ready → running` transition itself, so the same code path can never be used to smuggle a task into `running` outside the transition API. `voro continue <task-id>` exposes the mechanism directly, for retrying a continuation that failed (a dirty tree, a misconfigured agent) once the underlying problem is fixed, and `--no-dispatch` on `answer` opts out of continuation even when history exists. A true resume of the original session — `claude -p --resume` against a captured session id, say — is a natural follow-up if this generic fallback proves lossy in practice; it is deferred because the fallback works identically for any agent named in `agents.toml`, which a resume-based mechanism could not promise.
...
- if its task is still `running` *and the dead session is the task's most recent one*, the agent's process ended without calling `done` or `ask` — the session outcome is recorded (`capped` if the log tail matches a short list of known usage-limit phrases, else `failed`), but **the task is left `running`**. The reconciler deliberately does *not* auto-demote or re-queue it, because a session leaving its agent's listing (or a dead pid under a supervisor-owned launch) is indistinguishable from a normal completion whose `voro done` has not yet landed — so an automatic `running → ready` bounce fired on healthy, just-finished sessions as readily as on real deaths, yanking live work back into the queue. Instead the finalised session leaves the task as an orphaned `running` row (§9) — a `running` task with no live session, already surfaced by the running strip as needing attention — tagged with a distinct `reconcile` event naming the outcome. From there a human decides explicitly: `voro abort` drops it to `ready` (where the failed/capped session history now surfaces the redispatch flag, so a redispatch is one key away), `voro done` accepts it into `review`, or a redispatch/continue is issued. Genuinely-dead sessions (crash, usage-cap `SIGKILL`) thus still get *a* path — surfaced for the human — without an automatic re-queue that competes with a session that is actually still working. The most-recent guard still matters because a task can be aborted and redispatched while an earlier session's row is still open (its process outlived the abort): when that older process finally exits, its outcome is backfilled (`failed`/`capped`) with no event logged against the task, since a newer, live session now owns the running state and a dead predecessor must not touch it.
