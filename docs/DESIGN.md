...
Ordering of the queue: every `needs-input`, `review`, and `proposed` task plus the top five `ready` tasks — and any `ready` task flagged for redispatch (§8), which is exempt from that cap — in one list sorted by score. The exemption exists because a flagged task is a failed dispatch, exactly the attention the queue is for; without it a low-scoring flagged task would drop below the cap and vanish from the cockpit the refresh after it was visibly running. Because the state bonus now lives in the score, state usually settles itself; the state precedence (§6) only breaks genuinely equal totals. `voro next` still answers with the single top ready task — that is what an agent asking for work should be handed, and the dispatch default.
