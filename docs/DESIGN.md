...
- if its task is still `running` *and the dead session is the task's most recent one*, the agent's process ended without calling `done` or `ask` — the session outcome is `capped` if the log tail matches a short list of known usage-limit phrases, else `failed`, and the task drops `running → ready` flagged for redispatch. This reuses the `Abort` transition (same `running → ready` target, same cascading readiness checks) but tags the event log with a distinct `reconcile` entry naming the outcome, so an automatic reconciliation reads differently from a human's `voro abort`. The most-recent guard matters because a task can be aborted and redispatched while an earlier session's row is still open (its process outlived the abort): when that older process finally exits, its outcome is backfilled (`failed`/`capped`) but the task is left `running`, since a newer, live session now owns the running state and a dead predecessor must not demote it.
