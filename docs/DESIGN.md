...
Ordering of the queue: every `needs-input`, `review`, and `proposed` task plus a priority-driven selection of `ready` tasks, in one list sorted by score. The ready selection is not a strict count: every P0/P1 ready task always appears — startable work marked urgent or important is never truncated — and the highest-scoring P2/P3 ready tasks fill up to a floor of five rows, so the queue keeps a few startable options without spilling the low-priority backlog into it. Priority decides *whether* a ready row appears; score still decides *where* it sits.

An earlier design capped startable work at the few highest-scoring ready tasks, arguing a hard count kept the queue an answer rather than a todo list. That argument lost. A by-score count truncates on the wrong axis: a genuinely important ready task falls below the line whenever higher-weight, lower-priority work outscores it, so the operator's own priority marking stops governing what they see. The symptom was that the count kept needing to move — widened once from three to five (#33) — and then needing a per-task exemption carved through it (#74/PR #35) for exactly the important-but-outscored case above. Selecting by priority removes both the tuning and the exemptions: the quantity the operator cares about is the one that governs inclusion. The floor survives as an anti-boredom minimum for when everything startable is low priority, and the risk the cap guarded against — P0/P1 work flooding the queue — is now itself signal that prioritisation has slipped, the same guard rail the untriaged `proposed` rows provide (§12), rather than something to hide behind a count.
