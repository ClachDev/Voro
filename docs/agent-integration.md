...
- `cap` prints **when the account's usage window reopens**, as a Unix epoch in
  ...
  *refused*, print nothing otherwise, so an account with room left, a command
  ...
  reset time falls back to the clock time on the session's own screen, which is
  where it came from before this verb existed. Print a bare number; Voro reads
  the last plausible one and refuses anything that is not a timestamp near the
  present.
